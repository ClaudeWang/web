...
class ToDos extends React.Component {
    ...
    render() { return (
        <div>
            <input ref="newtodo" id="newTODO" type="text" placeholder="To Do"/>
            <button onClick={() => this.addTodo()}>Add Item</button>
            <span className="submit">
                <a href="https://webdev-rice.herokuapp.com" target="_blank">Submit your exercise</a>
            </span>
            <ul className="todo"> 
                {this.state.todoItems.map((item, index) => <ToDoItem key={item.id} text={item.text} remove={() => this.removeTodo(item.id) }/>)}              
            </ul>
        </div>
        /*
        h("div", { },
