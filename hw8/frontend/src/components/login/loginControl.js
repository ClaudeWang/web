import {resource, navToMain, url} from '../../actions'
...
function register(password, password_conf, dob, email, zipcode, username) {
	return (dispatch) => {
		...
		if (message.length == 0) {
			...
			dispatch(registerUser(username, email, zipcode, password, dob))
		} else {
		...
		.then(r => 
				{
					...
					dispatch({type: "loginType", loginType: "normal"})
				}
			)
...
function googleLogin() {
	return (dispatch) => {
		//tell the front end this is a google login
...
export {validateFields, login, updateWarningText, register, googleLogin}
