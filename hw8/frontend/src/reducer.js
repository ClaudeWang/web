...
function articles(state={articles: [], keyword: "", commentUpdate:1, uploadImage:null}, 
	action) {
	let newArticleList;
	switch (action.type) {
		case 'articles':
...
function loginType(state={loginType: "third", linked: "true"}, action) {
	switch (action.type) {
		case 'loginType':
...
const Reducer = combineReducers({route, updateProfile, follows, articles, 
	warningText, loginType})
export {route, updateProfile, follows, articles, warningText, loginType}
